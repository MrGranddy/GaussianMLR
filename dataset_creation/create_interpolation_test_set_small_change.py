...
brightness_interpolation = np.linspace(smallest_brightness, biggest_brightness, interpolation_steps)
...
for n_idx in range(num_tests):
    ...
    sample_path = os.path.join(ranked_mnist_path, "%d_%d_%d_%d" % (n_idx, sel_digits[0], sel_digits[1], sel_digits[2]))
    ...
    for t_idx in range(interpolation_steps):
        ...
        if interpolate == "brightness":
            ...
            three_brightness = smallest_brightness + biggest_brightness - brightness_interpolation[t_idx]
        ...
        one_scaled = (
            np.array(
                one_image.resize((int(28 * one_scale), int(28 * one_scale)), Image.BILINEAR)
            ).astype("float32")
            / 255
        )
        two_scaled = (
            np.array(
                two_image.resize((int(28 * two_scale), int(28 * two_scale)), Image.BILINEAR)
            ).astype("float32")
            / 255
        )
        three_scaled = (
        ...
        if mode == "gray":
            ...
            one_rand_hsv_color = (np.random.rand(), np.random.uniform(0.5, 1.0), one_brightness)
            two_rand_hsv_color = (np.random.rand(), np.random.uniform(0.5, 1.0), two_brightness)
            three_rand_hsv_color = (np.random.rand(), np.random.uniform(0.5, 1.0), three_brightness)
            ...
            one_scaled = one_scaled * np.expand_dims(np.array([*one_rand_rgb_color]), axis=(0, 1))
            two_scaled = two_scaled * np.expand_dims(np.array([*two_rand_rgb_color]), axis=(0, 1))
            three_scaled = three_scaled * np.expand_dims(np.array([*three_rand_rgb_color]), axis=(0, 1))
        ...
        Image.fromarray((canvas * 255).astype("uint8")).save(os.path.join(sample_path, "%d.png" % t_idx))
