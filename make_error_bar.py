...
with open("error_bar_results/%s_metrics.txt" % args.experiment_name, "w") as f:
    ...
    for key in run_scores["ranking"]:
        f.write("%s: mean: %.4f, std: %.4f\n" % ( key, np.mean(run_scores["ranking"][key]), np.std(run_scores["ranking"][key]) ))
    f.write("\n")
    f.write("Classification Metrics\n")
    for key in run_scores["classification"]:
        f.write("%s: mean: %.4f, std: %.4f\n" % ( key, np.mean(run_scores["classification"][key]), np.std(run_scores["classification"][key]) ))